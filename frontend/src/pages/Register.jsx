...
useEffect(() => {
  const fetchRegistrationStatus = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:5000/api/sports/registration-status/");
      setIsRegistrationOpen(response.data.isOpen); // Ensure backend returns {"isOpen": true/false}
    } catch (error) {
      console.error("Error fetching registration status:", error);
      setIsRegistrationOpen(false); // Default to closed if API fails
    }
  };
    ...
    <div>
      {!isRegistrationOpen ? (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-center text-blue-800 font-semibold text-3xl h-96 mt-10 p-4 border rounded-lg"
        >
